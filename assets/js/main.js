...
(function ($) {
	...
	(function () {
		...
		function onScrollDirection() {
			...
			if (Math.abs(delta) > 4) {
				if (currentY > activationOffset && delta > 0) {
					// scrolling down
					header.classList.add('da-header-hidden');
				} else {
					// scrolling up or near top
					header.classList.remove('da-header-hidden');
				}
				lastY = currentY;
			}
	...
	// Safely initialize jQuery UI slider only if the plugin and element exist
	if (typeof $.fn.slider === "function" && $("#slider-range").length) {
		$("#slider-range").slider({
			range: true,
			min: 0,
			max: 1200,
	...
	if ($('.tp-main-menu-content').length && $('.tp-main-menu-mobile').length) {
		...
		arrow.each(function () {
			...
			arrowBtn.innerHTML = "<i class='fa-light fa-angle-right'></i>";
	...
	// DA hamburger: animate icon & ensure offcanvas opens on all pages
	$(".da-burger").on("click", function () {
...
// DA global sound widget (wave + volume toggle)
$(function () {
	var widget = document.getElementById('da-sound-widget');
	var toggleBtn = document.getElementById('da-sound-toggle');
	var audioEl = document.getElementById('da-sound-audio');
	var STORAGE_KEY = 'da-sound-enabled';

	if (!widget || !toggleBtn || !audioEl) return;

	function setVisible(visible) {
		if (visible) {
			widget.classList.add('da-sound-visible');
			widget.setAttribute('aria-hidden', 'false');
		} else {
			widget.classList.remove('da-sound-visible');
	...
	function setActive(active) {
		if (active) {
			widget.classList.add('da-sound-active');
		} else {
			widget.classList.remove('da-sound-active');
		}
	}
	...
	// Restore previous preference (may still be blocked by browser autoplay rules)
	try {
	...
	// Click anywhere on the widget (wave area) to toggle sound
